...
int main()
{
 ...
 //Implementation of strcpy function
   ...
   //Implementation of strlen function
	...
	char str4[20];
	cout<<"Enter a string to compare with initial string."<<endl;
	cin.getline(str4,20);
    int result=compare(str,str4);//Compares the length of two strings
    if(result==0)
    cout<<"Strings are same"<<endl<<endl;
    else if(result>0)
    cout<<"The ascii value of the character of str after first mismatch is more than that of str2 with ascii difference:"<<result<<endl<<endl;
    else
    cout<<"The ascii value of the character of str after first mismatch is less than that of str2 with ascii difference:"<<result<<endl<<endl;
...
if(chr(p1,c)==NULL)
{
cout<<"This character is absent in the string"<<endl;
}
else{
while(chr(p1,c)!=NULL)
{
	cout<<"Found at position:"<<(chr(p1,c)-str+1)<<endl;
	p1=chr(p1,c)+1;
}
}
cout<<endl;
